    ...
    for m in words:
        ...
        if  fdist [m.lower()]==0 and m+'\n'  in wikiTitles and len(m.split())in [2,3]:
            ...
            if len(m.split())==2:
                ...
                # score for keyword m
                keyword_dict[m]=abs_occurences_dict[m]*2/(first_word_freq_score+second_word_freq_score+1)
            else:
                #===============================================================
                # first_word=m.split()[0]
                # second_word=m.split()[1]
                ...
                # # score for keyword m
                #===============================================================
                keyword_dict[m]=abs_occurences_dict[m]/(1)   
...
def combine_two_tag_dict(sorted_two_word_tags,sorted_one_word_tags,tag_to_text_ratio):
    print("Combining two tag list")
    
    # combining single word tags dict and two words tag dict 
    
    sorted_top_two_word_tags=islice(sorted_two_word_tags, tag_to_text_ratio)
...
def get_list_of_tags(text_file):
    ...
    with open(text_file,'r')as text:
        ...
        length_one_word_tag_list=len(sorted_one_word_tags)
    ...
    with open('../Wiki_titles/Wiki_titles.txt','rb')as titles:
        ...
        length_two_word_tag_list=len(sorted_two_word_tags)
        tag_to_text_ratio=int((length_one_word_tag_list+length_two_word_tag_list)*0.1)
        
    return  combine_two_tag_dict(sorted_two_word_tags,sorted_one_word_tags,tag_to_text_ratio)  